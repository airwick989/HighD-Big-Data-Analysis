...
def update_plot(car_frame_number, tracks_file, axis, fig, highway_image, west_bound_lane_lines, east_bound_lane_lines):
    ...
    highway_extent = [0, highway_length, min(west_bound_lane_lines), max(east_bound_lane_lines)]
...
fig, axis = plt.subplots(figsize=(32, 4))
...
writer = FFMpegWriter(fps=10, metadata=dict(artist='Me'), bitrate=1800, codec='libx264')

# Save the animation
ani.save('group6_animation.mp4', writer=writer, dpi=100)  # Specify the DPI if needed
